...
Original file is located at
    https://colab.research.google.com/drive/1a9X2Dj_Teg7PMRwR-HmY67Jd8fn-kAyC
"""
...
with tab1:
    ...
    with st.form(key="edit_form"):
        ...
        # --- Gestão das Aplicações Químicas (DENTRO do Formulário) ---
        st.subheader("Aplicações Químicas")

        # Exibe a lista de aplicações atuais
        if st.session_state.current_applications:
            st.markdown("**Aplicações a serem salvas:**")
            for i, app in enumerate(st.session_state.current_applications):
                col_app1, col_app2 = st.columns([4, 1])
                col_app1.info(f"**Tipo:** {app['tipo']} | **Volume:** {app['volume']} {app.get('unidade', 'un/ha')}")
                # Este botão não submete o formulário, apenas remove o item da lista na próxima execução
                if col_app2.form_submit_button("Remover", key=f"remove_app_{i}"):
        ...
        submitted = st.form_submit_button("Salvar Todas as Informações na Base de Dados", use_container_width=True)
        if submitted:
            photo_bytes = None
    ...
    # Inputs para adicionar nova aplicação (permanece fora do form para não submeter)
    with st.container(border=True):
        st.markdown("**Adicionar Nova Aplicação à Lista**")
        ...
        if col_add3.button("Adicionar à Lista", use_container_width=True):
            if 'current_applications' not in st.session_state:
                st.session_state.current_applications = []
